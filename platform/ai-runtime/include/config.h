...
struct Config {
    ...
    // Stream DSP preprocess (级2): when a StreamInfer frame's geometry does
    // not match the model input, resize it on the DSP into a private
    // model-geometry dmabuf pool and fd-bind that (zero CPU copies) instead
    // of failing at the HAL size check. Opt-in; off = legacy repack path.
