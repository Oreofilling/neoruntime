...
/**
 * @file dsp_client.h
 * @brief 级2 stream preprocess: camera-daemon DSP client for ai-runtime.
 *
...
public:
    ...
    /// fails or no slot is free (caller falls back to the repack path —
    /// the source frame is still held at that point). On success the
    /// source frame may be released immediately: the resized pixels live
    /// in the private slot. *dsp_us gets the resize job wall time.
