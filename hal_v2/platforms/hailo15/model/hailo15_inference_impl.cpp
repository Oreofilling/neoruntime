...
static int hailo15_bind_inputs_outputs(Hailo15InferPriv *p, const HalTensor *inputs, HalTensor *outputs)
{
    ...
    for (size_t i = 0; i < want_in; i++)
    {
        ...
        if (!in.data)
        {
            /* DMA-bound input from bind_dma_frame(): bind the dmabufs directly,
             * zero CPU pixels. The descriptor is re-applied on every submit —
             * works under submit_mtx and with future ping-pong bindings. */
            ...
            if (HAILO_SUCCESS != st)
            {
                HAL_LOG_ERROR("hailo15_inference: dma bind input[%zu] failed (st=%d)", i, (int)st);
