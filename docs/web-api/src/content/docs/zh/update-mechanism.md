...
同一 PR 内必须补齐 docs/web-api/i18n/zh.yaml 的中文翻译
        │
        ▼  合并到 main
GitHub Action docs-pages.yml 构建站点（Astro Starlight + Redoc）
        │
        ▼
GitHub Pages 自动重新部署
```
...
每当 `main` 上有触及 API 面、文档或工作流本身的推送，`docs-pages.yml` 会重新生成双语规范 JSON、重建 Starlight 站点并发布到 GitHub Pages。没有任何手工拷贝：`/api-reference/` 全屏参考页直接渲染 `swagger.json`（英文）与 overlay 合并生成的 `swagger.zh.json`（中文）（未翻译的字符串自动回退英文）。
...
cd docs/web-api
pnpm install
pnpm dev        # 重新生成规范 + 启动带热更新的 Starlight
```
...
| `docs/web-api/astro.config.mjs` | Starlight 站点配置（语言、侧边栏、主题）。 |
