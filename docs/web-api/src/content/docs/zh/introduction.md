...
API 按功能划分为以下分组（对应 [API 参考]((/neoruntime/api-reference/zh/)) 中的标签）：
...
- 所有响应共享一个信封：`{code, message, data, error}`。`code === 0` 表示成功，其余为业务错误码——见[错误与状态码]((/neoruntime/zh/errors/))。
- 除特别说明的端点（健康检查、公钥、登录、OTA 状态）外，所有端点都要求 Bearer 令牌——见[认证]((/neoruntime/zh/authentication/))。
...
见[文档同步机制]((/neoruntime/zh/update-mechanism/))。
