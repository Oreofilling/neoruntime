...
docs/web-api/i18n/zh.yaml must translate every operation in the same PR
        │
        ▼  merge to main
GitHub Action docs-pages.yml builds the site (Astro Starlight + Redoc)
        │
        ▼
...
cd docs/web-api
pnpm install
pnpm dev        # regenerate specs + start the Starlight dev server
```
...
| `docs/web-api/astro.config.mjs` | Starlight site configuration (locales, sidebar, theme). |
